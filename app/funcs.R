preprocess_weather_data <- function(weather_data) {
  weather_data %>%
    transmute(
      report_date,
...
preprocess_bike_data <- function(bike_data) {
  ...
  bike_data %>%
    ...
    # Only keep bike counts for the 5 sites that the 
    filter(site_name %in% c("Hollis St", "Windsor St", "South Park St", "Dartmouth Harbourfront Greenway", "Vernon St")) %>%
