...
bike_counts <- readRDS("bike-counts.rds")
bike_xgb_fit <- readRDS("bike-xgb-fit.rds")

#* apiTitle Predict bike ridership in Halifax, NS
#*
#* Uses a trained XGBoost model to predict the daily number of bicyclists
#* passing different counters ("sites") around the city.
#*
...
#* @param n_bikes_lag_14:int The number of bikes measured at the given `site_name` measured 14 days ago.
#* @param mean_temperature:numeric The daily mean temperature.
#* @param total_precipitation:numeric The daily amount of precipitation in mm.
#* @param snow_on_ground:numeric The daily amount of snow on the ground in cm.
#* @param speed_max_gust:numeric The daily maximum wind speed in km/h.
#* @get /n_bikes
function(count_date, site_name, n_bikes_lag_14 = NA_integer_,
  ...
  if (is.na(n_bikes_lag_14)) {
    ...
    bike_counts <- bike_counts %>% filter(site_name == .env$site_name)
  ...
  predict(bike_xgb_fit, d, type = "raw")
}
